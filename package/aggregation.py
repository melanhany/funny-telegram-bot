...
def aggregate_data(json_data):
    ...
    if (date_range["group_type"]=="hour"):
        ...
        result_month = agg_by_month(sample_collection, dt_from, dt_upto)
        result_dict = {}

        for r in result_month:
            result_dict[(r['_id']['year'], r['_id']['month'])] = r['total_value']

        # Add missing datetime records with 0 value
        curr_month = dt_from.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        end_month = dt_upto.replace(day=1, hour=0, minute=0, second=0, microsecond=0) + timedelta(days=31)
        iso_date_month = []
        total_value_month = []
        while curr_month <= end_month:
            iso_date_month.append(curr_month.isoformat())
            month_key = (curr_month.year, curr_month.month)
            if month_key in result_dict:
                total_value_month.append(result_dict[month_key])
            else:
                total_value_month.append(0)
